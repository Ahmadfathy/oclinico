...
@Component({
  selector: 'app-bankbranchmasterdetails',
  templateUrl: './bankbranchmasterdetails.component.html',
  styleUrls: ['./bankbranchmasterdetails.component.css']
})
export class BankbranchmasterdetailsComponent implements OnInit {
  userid: any;
  ...
  ngOnInit() {
    document.title="Bank BranchMaster Details";
   ...
   var accessToken =  window.localStorage.Tokenval;
      ...
      this.https.post(serviceUrl, params, options).map(res => res.json()).subscribe(result => {
        ...
        if (result.status_cd === "1") {
          this.bankbranchdetails = result.data.Table;
  ...
  edit(sno){
      ...
      this.router.navigate(['/editbankbranchmaster']);
  }
}
